...
app.get("/jokes/:id", (req, res) => {
 ...
 var foundJoke = jokes.find(joke => joke.id === idAsInt);
 res.json(foundJoke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filterByType = jokes.filter((joke) => joke.jokeType === typeOfJoke);
...
/*   const searchIndex = jokes.findIndex((joke) => joke.id === idAsInt);
...
app.patch("/jokes/:id", (req, res) => {
  const idAsInt = parseInt(req.params.id);
  const existingJoke = jokes.find (joke => joke.id === idAsInt);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({error: `Joke with id ${idAsInt} not found, no jokes removed`});
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if(key === masterKey) {
    jokes.length = 0;
    res.sendStatus(200);
    console.log("All jokes are now deleted");
  } else {
